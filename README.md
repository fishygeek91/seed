...
**Compare** (top bar) turns the sim into an experiment bench: pick a variant scenario — one changed cargo template, site, payload, or seed string — and it is simulated headlessly in the background to the same sol as your run, then overlaid on the history strip as a dashed amber curve with its own ×2 ticks. From there it becomes a **ghost race**: the variant keeps pace with your live run sol for sol, the amber curve growing beside yours while the verdict card (capacity, doublings, first wake, generation, imports) re-calls the race in real time. Try it on the demo seed: at sol 400, *Power first* beats *Balanced* by ~43% capacity, while *Hands first* never wakes a child at all.

**Film** (bottom bar) runs the mission reel and records it — the 3D feed composited with the letterbox, captions, and REC tag, plus the ambient audio when sound is on — into a downloadable `seed-reel-sol-N.webm`, entirely client-side via MediaRecorder.
